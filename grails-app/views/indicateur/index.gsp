		...
		<div id="list-indicateur" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="code" title="${message(code: 'indicateur.code.label', default: 'Code')}" />
					
						<g:sortableColumn property="definition" title="${message(code: 'indicateur.definition.label', default: 'Definition')}" />
				...
				<g:each in="${indicateurInstanceList}" status="i" var="indicateurInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${indicateurInstance.id}">${fieldValue(bean: indicateurInstance, field: "code")}</g:link></td>
					
						<td>${fieldValue(bean: indicateurInstance, field: "definition")}</td>
					
						<td>${fieldValue(bean: indicateurInstance, field: "nom")}</td>
